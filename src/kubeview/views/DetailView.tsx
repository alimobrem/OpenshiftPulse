...
/** Workload health audit shown on Deployment/StatefulSet/DaemonSet detail pages */
function WorkloadAudit({ resource, go }: { resource: K8sResource; go: (path: string, title: string) => void }) {
  const spec = resource.spec as any;
  ...
  const replicas = spec?.replicas ?? (resource.kind === 'DaemonSet' ? '—' : 0);
